...
def format_data():
    #data_places = db(db.places.id==request.vars.project_id).select(db.places.id, db.places.name)
    ...
    if form.process().accepted:
        ...
        results = db().select(db.topics.ALL, orderby=db.topics.priority|db.topics.id)
        for row in results:
        ...
        results = db().select(db.project_tree.topic, db.activities.name,
            ...
            left = db.activities.on(db.project_tree.activity==db.activities.id),
            orderby = db.project_tree.priority)
        for row in results:
            tree_data.setdefault(row.project_tree.topic, list()).append([row.project_tree.id,row.activities.name,row.activities.option_data])
        if project_id is not '' and place_id is not '':
            results = db((db.main_data.project==project_id)
                ...
                db.detail_data.element_tree, db.detail_data.choice,
                left = (db.detail_data.on(db.main_data.id==db.detail_data.reference),
                       db.groups.on(db.groups.id==db.detail_data.study_group),
                       db.individuals.on(db.individuals.id==db.detail_data.individual)
                    ),
            ...
            for row in results:
                #print row
                data_val.setdefault(row.detail_data.element_tree, list()).append(
